...
description: Library of composable baseline workflow definitions - the ping-pong build loop, the no-mistakes ship gate, and cross-cutting workflow primitives (goal anchors, evidence rules, gate semantics, refusal, tier routing, durability). Use when the user asks to generate, compose, design, or orchestrate a multi-agent workflow, wants a dynamic workflow built on the fly, or names a baseline like ping-pong or no-mistakes as the shape a workflow should follow.
license: MIT
---
...
Each file under `baselines/` defines one reusable workflow shape as concepts and invariants, not as a rigid script.
...
2. Load only the baseline files you picked, and skim `primitives.md` for graftable parts.
...
4. Carry every invariant from each loaded baseline into the composed workflow - as agent rules, verifier criteria, or gate checks.
5. Wire baselines at their contract seams: one baseline's exit contract must satisfy the next baseline's entry preconditions.
6. If the request is too thin to compose responsibly, bounce a sharp question instead of forcing a workflow (see the refusal primitive).
...
One file per baseline under `baselines/`, following the fixed anatomy: Intent, When to use, Roles or Stages, Contract, Invariants, Failure routing, Workflow skeleton, Composes with.
