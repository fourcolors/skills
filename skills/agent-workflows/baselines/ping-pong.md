...
- Evidence over assertion: the orchestrator re-runs the test on every return, and a claim without reproducible evidence is a rejected return.
...
Escalate to the human only at 3+ fruitless re-dispatches, a blown time-bound, or when the input itself proves wrong.
